...
This program will use the logistic regression algorithm on the 
...
We then normalize the data by standardization.

After the data has been normalized, we then utilize the sklearn
library in order to apply the logistic regression algorithm to our data.
...
def main():
    ...
    # numpy array so that we can utilize the np functions
    ...
    """
    In the below section, we utilize sklearns logistic
    regression algorithm in order to classify the
    data and find how accurate it is
    """
    ...
    accuracy = logit.score(test_features, test_targets)
